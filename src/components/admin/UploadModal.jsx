...
export default function UploadModal({
  ...
  const [selectedCategory, setSelectedCategory] = useState(
    preSelectedCategory || categories[0]?.id || "",
  );
  ...
  const processFiles = (selectedFilesList) => {
    ...
    selectedFiles.forEach((f) => {
      ...
      if (isPdf) {
        validFiles.push(f);
        if (isOverSizeLimit(f)) {
          largePdfFiles.push(f);
        }
      } else if (isWord) {
        if (isOverSizeLimit(f)) {
          oversizedWordFiles.push(f);
        } else {
          validFiles.push(f);
        }
      } else {
    ...
    if (rejectedFiles.length > 0) {
      warningMessages.push(
        `⚠️ ${rejectedFiles.length} tệp không thuộc định dạng PDF hoặc Word đã bị từ chối:\n• ${rejectedFiles.join("\n• ")}`,
      );
    }
    if (oversizedWordFiles.length > 0) {
      warningMessages.push(
        `⚠️ ${oversizedWordFiles.length} tệp Word vượt quá 48MB (Supabase giới hạn 50MB). Bạn hãy xuất sang PDF để hệ thống tự động nén tối ưu:\n• ${oversizedWordFiles.map((f) => `${f.name} (${(f.size / (1024 * 1024)).toFixed(1)}MB)`).join("\n• ")}`,
      );
    }
    if (largePdfFiles.length > 0) {
      warningMessages.push(
        `⚡ Phát hiện ${largePdfFiles.length} tệp PDF > 48MB. Hệ thống sẽ TỰ ĐỘNG NÉN tối ưu dung lượng khi bạn bấm Tải lên:\n• ${largePdfFiles.map((f) => `${f.name} (${(f.size / (1024 * 1024)).toFixed(1)}MB)`).join("\n• ")}`,
      );
    }
  ...
  const handleSubmit = async (e) => {
    ...
    const targetSubId =
      selectedCategory === "cat-general" ? null : selectedSubFolder || null;
    ...
    try {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        setProgressPercent(Math.max(basePercent + 5, 10));

        let fileToUpload = file;
        const originalMb = (file.size / (1024 * 1024)).toFixed(1);

        // Tự động nén nếu file PDF vượt ngưỡng 48MB (Supabase Free limit 50MB)
        if (isPdfFile(file) && isOverSizeLimit(file)) {
          setUploadStepText(
            `[Tệp ${i + 1}/${files.length}] File ${originalMb}MB > 48MB: Đang tự động nén tối ưu...`,
          );
          try {
            const compResult = await compressPdfFile(file, {}, (p) => {
              const fileBase = Math.round((i / files.length) * 100);
              const slice = Math.round((p.percent / files.length) * 0.45);
              setProgressPercent(Math.min(fileBase + slice, 90));
              setUploadStepText(
                `[Tệp ${i + 1}/${files.length}] ${p.statusText}`,
              );
            });
            fileToUpload = compResult.compressedFile;
            compressedFilesCount++;
            const newMb = (compResult.compressedSize / (1024 * 1024)).toFixed(
              1,
            );
            console.log(
              `Đã nén thành công ${file.name}: ${originalMb}MB -> ${newMb}MB (-${compResult.ratio}%)`,
            );
          } catch (compErr) {
            console.warn("Lỗi nén PDF tự động:", compErr);
            if (file.size > 50 * 1024 * 1024) {
              storageErrors.push(
                `${file.name}: Tệp quá 50MB và nén thất bại (${compErr.message})`,
              );
              continue;
            }
          }
        ...
        setProgressPercent(Math.max(basePercent + 20, 30));
        setUploadStepText(
          `[Tệp ${i + 1}/${files.length}] Đang tải lên Cloud & trích xuất chữ...`,
        );
        ...
        const [extractedContentResult, uploadStorageResult] = await Promise.all(
          [
            ...
            uploadPdfFileToStorage(fileToUpload).catch((err) => {
        ...
        if (!uploadStorageResult?.url) {
          storageErrors.push(
            `${file.name}: ${uploadStorageResult?.error || "Lỗi Storage"}`,
          );
        }
        ...
        setUploadStepText(
          `[Tệp ${i + 1}/${files.length}] Đang đồng bộ vào kho dữ liệu...`,
        );
        ...
        const newDoc = {
          ...
          fileType: fileType,
          tags:
            tagsArray.length > 0 ? tagsArray : ["NSG", fileType.toUpperCase()],
          createdAt: new Date().toISOString().split("T")[0],
        };

        // Lưu trực tiếp file nhị phân vào IndexedDB để không bị mất khi refresh trình duyệt
        await saveLocalFile(newDoc.id, fileToUpload).catch((err) => {
      ...
      if (onShowToast) {
        if (storageErrors.length > 0) {
          onShowToast(
            `Đã lưu ${files.length} tệp, nhưng ${storageErrors.length} tệp chưa lên được Cloud Supabase (${storageErrors[0]}).`,
            "warning",
          );
        } else {
          const compNote =
            compressedFilesCount > 0
              ? ` (Đã tự động nén tối ưu ${compressedFilesCount} tệp > 48MB)`
              : "";
          onShowToast(
            `Đã upload và đồng bộ thành công ${files.length} tài liệu vào kho Cloud!${compNote}`,
            "success",
          );
        }
      }
  ...
  return (
    <div className="fixed inset-0 z-50 bg-zinc-950/70 backdrop-blur-xs flex items-center justify-center p-0 sm:p-4 animate-in fade-in duration-200">
      <div className="bg-white border-0 sm:border border-zinc-200 w-full max-w-lg h-full sm:h-auto rounded-none sm:rounded-2xl shadow-2xl overflow-hidden flex flex-col">
        {/* Header */}
        <div className="px-4 sm:px-5 py-3.5 sm:py-4 bg-[#504b44] text-white flex items-center justify-between shrink-0">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="font-semibold text-sm text-white">
                Upload Nhiều File Tài liệu Hàng loạt
              </h3>
              <p className="text-[11px] text-zinc-300">
                Tải nhiều tệp PDF &amp; Word (.docx) cùng lúc vào kho NSG
              </p>
            </div>
          </div>
          ...
          <button
            onClick={onClose}
            disabled={uploading}
            className="p-1.5 text-zinc-300 hover:text-white hover:bg-[#625d55] rounded-lg transition-colors disabled:opacity-50 cursor-pointer"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <form
          ...
          className="p-4 sm:p-5 space-y-4 max-h-full sm:max-h-[75vh] overflow-y-auto text-xs flex-1"
        >
          ...
          <div>
            ...
            >
              ...
              />
              <label
                htmlFor="doc-upload-input-multi"
                className="cursor-pointer flex flex-col items-center gap-1.5"
              >
                <Upload className="w-8 h-8 text-[#d0aa61]" />
                <span className="font-semibold text-zinc-800">
                  {dragActive
                    ? "Kéo thả các file PDF / Word vào đây"
                    : "Click hoặc Kéo thả nhiều tệp file cùng lúc"}
                </span>
                <span className="text-[10px] text-zinc-400">
                  Chỉ chấp nhận file PDF (.pdf) hoặc Word (.doc, .docx)
                </span>
              </label>
            </div>
          </div>
          ...
          {files.length > 0 && (
            <div className="space-y-1.5 border border-zinc-200 rounded-xl p-3 bg-zinc-50/80 max-h-40 overflow-y-auto">
              ...
              {files.map((f, idx) => {
                const isLargePdf = isPdfFile(f) && isOverSizeLimit(f);
                return (
                  <div
                    key={idx}
                    className="flex items-center justify-between bg-white px-2.5 py-1.5 rounded-lg border border-zinc-200 text-xs"
                  >
          ...
          <div>
            ...
            <input
              type="text"
              placeholder="Nhập các thẻ cách nhau bằng dấu phẩy (VD: Concept, Branding, 2026...)"
              value={tagsInput}
